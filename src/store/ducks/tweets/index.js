...
export function tweetsReducer(state = INITIAL_STATE, action = {}) {
  ...
  if (action.type === 'tweets/LIKE') {
    ...
    const stateParcial = state.data.reduce((stateParcial, tweetAtual) => {
      ...
      if (isLikedTweet) {
        ...
        stateParcial.data = [...stateParcial.data, updateTweet]
        stateParcial.activeDataItem = Object.keys(stateParcial.activeDataItem).length ? updateTweet : stateParcial.activeDataItem
      } else {
        stateParcial.data = [...stateParcial.data, tweetAtual]
      }

      return stateParcial
    }, { activeDataItem: state.activeDataItem, data: [] })
